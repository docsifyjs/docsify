 ...
 * @return {String}
 */
var renderTree = function (tree, tpl) {
  if ( tpl === void 0 ) tpl = '<ul>';
 ...
 * @return {String}
 */
function tree (toc, opts) {
  var _tree = Array.isArray(opts.sidebar)
    ? opts.sidebar
...
var render = function (content, opts) {
  ...
  var corner$$1 = opts.repo ? corner(opts.repo) : '';
  var article$$1 = article(marked(content));
  var sidebar$$1 = sidebar(tree(toc, opts));

  return corner$$1 + main({ article: article$$1, sidebar: sidebar$$1 })
};
...
Docsify.prototype.run = function run () {
    var this$1 = this;

  var dom = document.querySelector(this.opts.el) || document.body;
  var replace = dom !== document.body;
  var loc = document.location.pathname;

  var renderTo = function (content) {
      if ( content === void 0 ) content = 'not found';

    dom[replace ? 'outerHTML' : 'innerHTML'] = render(content, this$1.opts);
  };
  ...
  .catch(function (_) { return renderTo(); });
};

var index = new Docsify();
