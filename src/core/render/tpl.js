  ...
  const aside = (
    <>
      ...
      <aside class="sidebar">
        {config.name && (
          <h1 class="app-name">
            <a class="app-name-link" data-nosearch>
              {config.logo ? <img alt={name} src={config.logo} /> : name}
            </a>
          </h1>
        )}
        <div class="sidebar-nav" innerHTML={'<!--sidebar-->'}></div>
      </aside>
    </>
  );
...
/**
 * This is used in the markdown compile to generate paragraph HTML strings for
 * the markdown compiler (marked).
 */
